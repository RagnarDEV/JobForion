...
(function(){
  ...
  // BATCHING: every event used to be its own POST (a listing page with 20
  // job cards fired 20+ requests, each costing D1 writes and eating the
  // per-IP rate limit). Events are now queued and sent as ONE {events:[...]}
  // beacon after 2.5s idle, at 20 queued events, or when the page is hidden.
  var BATCH_MAX=20,queue=[],timer=null;
  function post(batch){var body=JSON.stringify({events:batch});try{if(navigator.sendBeacon&&navigator.sendBeacon('/api/analytics/events',new Blob([body],{type:'application/json'}))) return;fetch('/api/analytics/events',{method:'POST',headers:{'Content-Type':'application/json'},body:body,keepalive:true}).catch(function(){});}catch(e){}}
  function flush(){if(timer){clearTimeout(timer);timer=null;}while(queue.length){post(queue.splice(0,BATCH_MAX));}}
  function schedule(){if(!timer){timer=setTimeout(function(){timer=null;flush();},2500);}}
  document.addEventListener('visibilitychange',function(){if(document.visibilityState==='hidden')flush();});
  window.addEventListener('pagehide',flush);
  function send(type, data){ if(!allowed.has(type)) return; var payload=Object.assign({event_type:type,event_id:(crypto.randomUUID?crypto.randomUUID():String(Date.now())+'_'+Math.random()),session_id:sessionId,page:location.pathname,landing_page:location.pathname,referrer:document.referrer||'',device_type:device(),source:utm.source||'direct',medium:utm.medium||'none',campaign:utm.campaign||''},data||{}); var key=type+'|'+(payload.job_id||'')+'|'+(payload.page||''); if((type==='page_view'||type==='job_view')&&sent.has(key))return; sent.add(key); queue.push(payload); if(queue.length>=BATCH_MAX){flush();}else{schedule();} }
