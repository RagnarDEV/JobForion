...
// True when the request carries NO account-session cookie, i.e. the rendered
// HTML is identical for every such visitor and safe to serve from the edge
// cache. Unrelated cookies (Google Analytics' _ga, consent banners, the admin
// cookie) must NOT defeat caching, so this checks the session cookie only.
