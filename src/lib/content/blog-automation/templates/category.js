// src/lib/content/blog-automation/templates/category.js
...
import { escapeHtml, slugify, categorySalaryStats } from '../../../directory/entities.js';
import { JOB_MANUAL_PIN_SORT_SQL, PUBLIC_JOB_STATUS_SQL } from '../../../../config/constants.js';
...
export async function render(env, candidate, settings, base) {
  ...
  const { results: jobs } = await env.DB.prepare(
    `SELECT * FROM jobs WHERE LOWER(title) LIKE ? AND ${PUBLIC_JOB_STATUS_SQL} ORDER BY ${JOB_MANUAL_PIN_SORT_SQL} LIMIT ?`
