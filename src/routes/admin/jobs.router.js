...
import { salaryClassificationForJob } from '../../lib/jobs/salary-tier.js';
...
export async function handleAdminJobsRoute(url, request, env, base) {
      ...
      if (id) {
        ...
        if (p) {
          try {
            ...
            // through the same lib/jobs/salary.js used for every provider-
  ...
  if (url.pathname === '/admin/jobs/feature' && request.method === 'POST') {
    try {
      const ok = await verifyAdminCookie(env, request.headers.get('Cookie'));
      if (!ok) return new Response('Unauthorized', { status: 401 });
      // Feature Flag: Featured Jobs (see lib/platform/settings.js). Blocked at the
