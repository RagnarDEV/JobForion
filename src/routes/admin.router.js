...
export async function handleAdminRoute(url, request, env, base) {
  ...
  // CENTRAL AUTH GATE (defense in depth): nothing below /admin is reachable
  // without a valid signed admin cookie except the login/logout endpoints.
  // Sub-routers still re-check individually, but a forgotten check in a
  // future sub-router can no longer expose an admin route.
  if (!isAuthenticated && url.pathname !== '/admin/login' && url.pathname !== '/admin/logout') {
    if (request.method === 'GET' || request.method === 'HEAD') {
      return new Response(renderAdminLogin(false), { headers: { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'no-store' } });
    }
