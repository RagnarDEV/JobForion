...
import { getPostById, getPostBySlug, wasAutoPostSlug } from '../lib/content/blog-cms.js';
import { RESERVED_SLUGS } from '../lib/content/pages-cms.js';
import { baseLayout } from '../layout/base-layout.js';
import { PUBLIC_JOB_STATUS_SQL } from '../config/constants.js';
import { getSettings } from '../lib/platform/settings.js';
import { getCategoryData } from '../lib/content/categories.js';
import { getSessionUser, isAnonymousRequest } from '../lib/accounts/session.js';
import { normalizedCacheKey, readEdgeCache, writeEdgeCache, CACHE_PRESETS } from '../lib/platform/cache.js';
import { renderPricingPage } from '../pages/pricing.js';

import { iconClock, iconSearch } from '../assets/icons.js';
...
<div class="page-sm" style="text-align:center;padding-top:60px">
  <div style="font-size:56px;margin-bottom:8px;opacity:.35">${isGone ? `${iconClock({ size: 15 })}` : `${iconSearch({ size: 15 })}`}</div>
...
// Blog Automation (src/lib/content/blog-automation/expiration.js) hard-deletes
...
async function renderBlogGonePage(env, base, slug) {
  ...
  const content = `
<div class="page-sm" style="text-align:center;padding-top:60px">
  <div style="font-size:56px;margin-bottom:8px;opacity:.35">${wasReal ? `${iconClock({ size: 15 })}` : `${iconSearch({ size: 15 })}`}</div>
  <h1 style="font-family:'Plus Jakarta Sans',sans-serif;font-size:24px;font-weight:800;color:var(--ink);margin-bottom:10px">${headline}</h1>
  <p style="color:var(--ink2);font-size:14px;line-height:1.7;max-width:420px;margin:0 auto 28px">${body}</p>
...
export async function handlePagesRoute(url, request, env, base, ctx = null) {
  // (The anonymous-homepage cache lookup happens once, in src/index.js,
  // before schema bootstrap — repeating it here only doubled the miss cost.)
  const session = await getSessionUser(env, request);
  const user = session?.user || null;

  const jobMatch = url.pathname.match(/^\/job\/(\d+)$/);
  if (jobMatch) {
    // Anonymous visitors (and crawlers) all get identical HTML, so the 200
    // page is edge-cached for 5 minutes. 404/410 branches are never cached.
    const jobCacheKey = request.method === 'GET' && !user ? normalizedCacheKey(request, []) : null;
    if (jobCacheKey) {
      const hit = await readEdgeCache(jobCacheKey);
      if (hit) return hit;
    }
    ...
    if (jobCacheKey) writeEdgeCache(ctx, jobCacheKey, jobResponse);
    return jobResponse;
  }
  ...
  // lib/content/pages-cms.js) can never be used as a page slug, so this only
