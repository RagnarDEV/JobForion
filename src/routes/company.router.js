...
import { parseSalary } from '../lib/jobs/salary.js';
import { findUserByEmail } from '../lib/accounts/users.js';
import {
  createCompany, getCompanyById, updateCompanyProfile, addCompanyMember, removeCompanyMember,
} from '../lib/companies/companies.js';
...
export async function handleCompanyRoute(url, request, env, base) {
  ...
  // site-wide (lib/companies/company-logos.js overrides).
  ...
  if (url.pathname === '/company/post-job') {
    ...
    if (salaryMinRaw || salaryMaxRaw) {
      ...
      // Canonical human-readable string, re-parsed by lib/jobs/salary.js's
