...
import { iconMail, iconSparkle } from '../assets/icons.js';
...
// caller not yet passing dynamic categories (see lib/content/categories.js).
...
<div class="pj-overlay" id="pjOverlay" onclick="if(event.target===this)closePostJobModal()">
  <div class="pj-modal">
    ...
    <div id="pjFormWrap">
      <div class="pj-title">${iconMail({ size: 15 })} Post a Remote Job</div>
...
async function submitPostJob(e){
  ...
  try{
    ...
    if(d.success){
      document.getElementById('pjFormWrap').innerHTML='<div class="pj-success"><div class="ico">${iconSparkle({ size: 44 })}</div><div class="pj-title">Thanks — received!</div><div class="pj-sub">Our team will review your listing and publish it within 24 hours.</div><button class="pj-submit" onclick="closePostJobModal()">Done</button></div>';
    }else{
