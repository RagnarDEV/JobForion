...
import { DEFAULT_CARD_STYLES, buildCardStyleAttr, buildBadgeStyleAttr, jobTypeIconHtml } from '../lib/jobs/job-card-styles.js';
import { logoProxyPath } from '../lib/companies/logo-proxy.js';
import { isHotPayJob, HOT_PAY_LABEL } from '../lib/jobs/hot-pay.js';
import { salaryTierEnabled } from '../lib/jobs/salary-tier.js';
...
export function logoImgHtml(company, size = '64px', cls = 'job-logo', overrideUrl = null, website = '') {
  ...
  // lib/companies/logo-proxy.js so the visitor's browser only ever talks to our own
...
// lib/content/categories.js) behaves exactly as before. Callers that already
...
// DEFAULT_CARD_STYLES (see lib/jobs/job-card-styles.js) so any caller not
...
// Compatibility wrapper for the single HOT PAY rule in lib/jobs/hot-pay.js.
...
// Sponsored} object from lib/jobs/job-card-styles.js — defaults to
...
// 3) mirrors the `feature_featured_jobs` flag in lib/platform/settings.js. When
...
// admin-verified companies row (see lib/companies/companies.js's
