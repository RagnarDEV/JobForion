...
// See lib/content/blog-cms.js for the data layer. Category and tags are kept as
...
import { getContentAnalysis } from '../../lib/ai/content-intelligence.js';

import { iconBot, iconEdit3, iconFileText } from '../../assets/icons.js';
const STATUS_BADGE = {
...
export async function renderBlogListContent(env) {
  ...
  const AUTO_BADGE = `<span style="font-size:10px;font-weight:800;color:var(--brand);background:var(--brand-soft);padding:2px 8px;border-radius:20px;margin-left:4px">${iconBot({ size: 15 })} AUTO</span>`;
  ...
  <div class="adm-wrap">
    <div class="adm-hdr">
      <div>
        <div class="adm-title">${iconFileText({ size: 22 })} Blog</div>
...
function postForm(post, isNew, analysis = null) {
  ...
  <div class="adm-wrap" style="max-width:820px">
    <div class="adm-hdr">
      <div>
        <div class="adm-title">${isNew ? `${iconFileText({ size: 22 })} New Article` : `${iconEdit3({ size: 22 })} Edit — ${escapeHtml(p.title)}`}</div>
        ${!isNew && p.auto_generated ? `<div class="adm-sub">${iconBot({ size: 15 })} Auto-generated by Blog Automation${p.source_type ? ` (${escapeHtml(p.source_type)})` : ''}</div>` : ''}
      </div>
