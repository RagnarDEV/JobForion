...
// per-slot code, enable/disable, and box size — see lib/content/ad-slots.js for
...
import { getAdSlotsConfig, AD_SLOT_DEFS } from '../../lib/content/ad-slots.js';
import { getSettings } from '../../lib/platform/settings.js';

import { iconMegaphone } from '../../assets/icons.js';
function slotCard(def, slot) {
  ...
  <div class="adm-card" style="margin-bottom:16px${slot.enabled ? '' : ';opacity:.6'}">
    <div class="adm-card-title" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px">
      <span><span class="health-dot ${slot.enabled ? 'health-ok' : 'health-off'}"></span> ${escapeHtml(def.label)}</span>
...
export async function renderAdsContent(env) {
  ...
  <div class="adm-wrap" style="max-width:760px">
    <div class="adm-hdr">
      <div>
        <div class="adm-title">${iconMegaphone({ size: 22 })} Ads</div>
