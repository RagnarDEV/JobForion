...
import { getRecentActivity, ACTION_LABELS } from '../../lib/platform/activity-log.js';
import { getSettings } from '../../lib/platform/settings.js';
import { hotPayThresholdUsd } from '../../lib/jobs/hot-pay.js';
import { getAnalyticsHealth } from '../../lib/analytics/events.js';
import { paymentProviderStatus } from '../../lib/monetization/core.js';

import { iconAlertTriangle, iconBriefcase, iconBuilding, iconHome, iconMail, iconMegaphone, iconPlug, iconServer, iconSettingsGear, iconShieldCheck, iconTrash2 } from '../../assets/icons.js';
...
@media(max-width:640px){.dashboard-page{width:100%;max-width:none;min-width:0}.dashboard-page .dashboard-kpi-grid,.dashboard-page .dashboard-pulse-strip,.dashboard-page .dashboard-overview-grid,.dashboard-page .dashboard-lower-grid{width:100%;max-width:none;min-width:0}.dashboard-page .dashboard-overview-grid,.dashboard-page .dashboard-lower-grid{grid-template-columns:minmax(0,1fr)!important}.dashboard-page .dashboard-kpi,.dashboard-page .dashboard-pulse,.dashboard-page .dashboard-overview-grid>.adm-card,.dashboard-page .dashboard-lower-grid>.adm-card{width:100%;min-width:0}.dashboard-page .dashboard-overview-grid>.adm-card,.dashboard-page .dashboard-lower-grid>.adm-card{grid-column:1 / -1!important}}
  </style>`;
  ...
  <div class="adm-wrap dashboard-page">
    <div class="adm-hdr">
      ...
      <div class="dashboard-actions">
        ...
        <form method="POST" action="/admin/cleanup" onsubmit="return confirm('Run cleanup now? It advances expired jobs through the retention lifecycle; only archived jobs past retention are permanently deleted.')" style="display:inline">
          <button class="adm-btn" type="submit" style="border-color:var(--coral);color:var(--coral)">${iconTrash2({ size: 15 })} Run Cleanup Now</button>
        </form>
    ...
    <div class="adm-card dashboard-section-card" style="margin-bottom:16px">
      ...
      <div class="adm-quick-grid">
        <a class="adm-quick-card" href="/admin/sources">${iconPlug({ size: 18 })} <span>Manage sources</span></a>
        <a class="adm-quick-card" href="/admin/jobs">${iconBriefcase({ size: 18 })} <span>Manage jobs</span></a>
        <a class="adm-quick-card" href="/admin/companies">${iconBuilding({ size: 18 })} <span>Manage companies</span></a>
        <a class="adm-quick-card" href="/admin/homepage">${iconHome({ size: 18 })} <span>Homepage sections</span></a>
        <a class="adm-quick-card" href="/admin/ads">${iconMegaphone({ size: 18 })} <span>Manage ads</span></a>
        <a class="adm-quick-card" href="/admin/settings">${iconSettingsGear({ size: 18 })} <span>Website settings</span></a>
        <a class="adm-quick-card" href="/admin/system">${iconServer({ size: 18 })} <span>System health</span></a>
        <a class="adm-quick-card" href="/admin/security">${iconShieldCheck({ size: 18 })} <span>Activity log</span></a>
      </div>
    </div>
    ...
    <div class="adm-card dashboard-pending-card" style="margin-bottom:16px">
      <div class="adm-card-title">${iconMail({ size: 16 })} Pending Job Postings <span style="font-weight:400;color:var(--ink3);font-size:12px">— submitted via "Post a Job"</span></div>
      ...
      <div class="adm-card dashboard-compact-card">
        ...
        <div class="dashboard-compact-list">${(syncLogs || []).length ? syncLogs.map(s => {
          ...
          return `<div class="dashboard-sync-row">
            ...
            ${errs.length ? `<div class="dashboard-sync-errors">${errs.map(e => `<div>${iconAlertTriangle({ size: 15 })} ${escapeHtml(String(e))}</div>`).join('')}</div>` : ''}
          </div>`;
      ...
      <div class="adm-card dashboard-compact-card">
        ...
        ${(cleanupLogs || []).length ? cleanupLogs.map(c => {
          ...
          return `<div class="adm-row" style="align-items:flex-start;flex-direction:column;gap:4px">
            ...
            ${breakdown.error ? `<div style="font-size:10px;color:var(--coral)">${iconAlertTriangle({ size: 15 })} ${escapeHtml(breakdown.error)}</div>` : ''}
          </div>`;
    ...
    <div class="adm-card dashboard-section-card" style="margin-top:16px">
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap">
        ...
        <a href="/admin/sources" class="adm-btn adm-btn-primary">${iconPlug({ size: 15 })} Manage Job Sources</a>
      </div>
    </div>
  </div>`;
