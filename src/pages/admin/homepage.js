...
import { getAllHomepageSections } from '../../lib/content/homepage-sections.js';
import { getAllHomepageCustomSections, HOMEPAGE_CUSTOM_SECTION_LIMITS } from '../../lib/content/homepage-custom-sections.js';
import { pageCodeEditorHtml } from '../../components/page-code-editor.js';

import { iconEdit3, iconHome, iconLayoutGrid } from '../../assets/icons.js';
export async function renderHomepageBuilderContent(env) {
  ...
  <div class="adm-wrap">
    <div class="adm-hdr">
      <div>
        <div class="adm-title">${iconHome({ size: 22 })} Homepage Sections</div>
...
function customSectionForm(section, isNew) {
  ...
  <div class="adm-wrap" style="max-width:980px">
    <div class="adm-hdr"><div><div class="adm-title">${isNew ? `${iconLayoutGrid({ size: 22 })} New Homepage Section` : `${iconEdit3({ size: 15 })} Edit — ${escapeHtml(s.title)}`}</div><div class="adm-sub">${isNew ? 'Add a custom block to the homepage' : 'Update this custom homepage block'}</div></div><a href="/admin/homepage" class="adm-btn">Back</a></div>
...
function homepageSectionCodeForm(section) {
  ...
  return `
  <div class="adm-wrap" style="max-width:980px">
    <div class="adm-hdr"><div><div class="adm-title">${iconEdit3({ size: 22 })} Edit Code — ${escapeHtml(section?.label || key)}</div><div class="adm-sub">Edit this existing Homepage section directly. Empty code fields keep the original JobForion renderer for this section.</div></div><a href="/admin/homepage" class="adm-btn">Back</a></div>
