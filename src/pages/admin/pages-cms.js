...
// reorder, delete. See lib/content/pages-cms.js for the data layer and the
// reserved-slug / effective-status rules.

import { escapeHtml } from '../../lib/directory/entities.js';
import { getPages } from '../../lib/content/pages-cms.js';
import { getAllNavButtons } from '../../lib/content/nav-buttons.js';
import { richEditorHtml } from '../../components/rich-editor.js';
import { pageCodeEditorHtml } from '../../components/page-code-editor.js';

import { iconEdit3, iconFileText, iconLink } from '../../assets/icons.js';
const STATUS_BADGE = {
...
export async function renderPagesListContent(env) {
  ...
  <div class="adm-wrap">
    <div class="adm-hdr">
      <div>
        <div class="adm-title">${iconFileText({ size: 22 })} Pages</div>
    ...
    <div class="adm-card">
      <div class="adm-card-title">${iconLink({ size: 16 })} Custom Menu Buttons <span style="font-weight:400;color:var(--ink3);font-size:12px">— arbitrary buttons in the mobile/nav menu: internal path or external URL, custom icon, custom color</span></div>
...
function pageForm(page, isNew) {
  ...
  <div class="adm-wrap" style="max-width:820px">
    <div class="adm-hdr">
      <div>
        <div class="adm-title">${isNew ? `${iconFileText({ size: 22 })} New Page` : `${iconEdit3({ size: 22 })} Edit — ${escapeHtml(p.title)}`}</div>
