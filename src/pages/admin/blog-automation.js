...
// lib/content/blog-automation/logger.js.

import { escapeHtml } from '../../lib/directory/entities.js';
import { getSettings, WEEKDAY_OPTIONS } from '../../lib/platform/settings.js';
import { getBlogAutomationStats, getRecentBlogEvents, BLOG_EVENT_LABELS } from '../../lib/content/blog-automation/logger.js';
import { TEMPLATES } from '../../lib/content/blog-automation/templates/index.js';

import { iconBot, iconClock, iconEdit3, iconFileText, iconFolder, iconRefreshCw, iconTrash2 } from '../../assets/icons.js';
...
export async function renderBlogAutomationContent(env) {
  ...
  <div class="adm-wrap">
    <div class="adm-hdr">
      <div>
        <div class="adm-title">${iconBot({ size: 22 })} Blog Automation</div>
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        ...
        <form method="POST" action="/admin/blog-automation/expire-now" onsubmit="return confirm('Run expiration cleanup now? This permanently deletes any due auto-generated articles.')" style="display:inline">
          <button class="adm-btn" type="submit" style="border-color:var(--coral);color:var(--coral)">${iconTrash2({ size: 15 })} Run Expiration Now</button>
        </form>
        <a href="/admin/blog" class="adm-btn">${iconFileText({ size: 15 })} View in Blog CMS</a>
      </div>
    </div>
    ...
    <form method="POST" action="/admin/blog-automation/update" style="display:flex;flex-direction:column;gap:16px">
      ...
      <div class="adm-card">
        <div class="adm-card-title">${iconClock({ size: 16 })} Schedule</div>
      ...
      <div class="adm-card">
        <div class="adm-card-title">${iconFolder({ size: 16 })} Topics <span style="font-weight:400;color:var(--ink3);font-size:12px">— which article types the topic engine may choose from</span></div>
      ...
      <div class="adm-card">
        <div class="adm-card-title">${iconEdit3({ size: 16 })} Content</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
      ...
      <div class="adm-card">
        <div class="adm-card-title">${iconRefreshCw({ size: 16 })} Duplicate Protection &amp; Expiration</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
