...
import { iconBriefcase, iconBuilding, iconBookmark, iconFileText, iconArrowRight, iconGlobe } from '../assets/icons.js';
...
export async function renderInformationalPage(slug, base, env, user = null) {
  ...
  const definitions = {
    ...
    contact: {
      ...
      sections: `<section class="public-card-grid"><article class="public-card"><span class="public-card-icon" aria-hidden="true">${iconGlobe({ size: 18 })}</span><h2>Email JobForion</h2><p>Reach the configured contact address for general questions and platform feedback.</p><div class="public-card-meta"><small>Configured contact</small><b class="public-card-arrow" aria-hidden="true">→</b></div></article><article class="public-card"><span class="public-card-icon" aria-hidden="true">${iconBriefcase({ size: 18 })}</span><h2>Post a job</h2><p>Employers can use the existing posting flow without a separate contact form.</p><div class="public-card-meta"><small>Employer flow</small><b class="public-card-arrow" aria-hidden="true">→</b></div></article></section>`,
    },
  };
  const definition = definitions[slug];
  if (!definition) return null;
  const contact = slug === 'contact' ? emailValue(settings.contact_email) : '';
  const contactAction = contact ? `<a class="public-primary-link" href="mailto:${escapeHtml(contact)}">${escapeHtml(contact)} ${iconArrowRight({ size: 14 })}</a>` : '';
  const employerAction = slug === 'contact' ? `<button class="public-primary-link" type="button" onclick="openPostJobModal()">Post a job ${iconArrowRight({ size: 14 })}</button>` : '';
...
export async function renderNotFoundPage(base, env, user = null) {
  ...
  const content = `<div class="page public-page not-found-page">${PUBLIC_PAGE_CSS}${bc}<div class="not-found-code">404</div><div class="public-page-header"><div class="public-page-header-copy"><p class="public-eyebrow">PAGE NOT FOUND</p><h1>We couldn’t find that page</h1><p>The address may be outdated or the content may have moved. Use one of the paths below to continue.</p></div></div><div class="public-card-grid"><a class="public-card" href="/"><span class="public-card-icon">⌂</span><h2>Go home</h2><p>Return to the JobForion homepage.</p><span class="public-card-meta"><small>Home</small><b class="public-card-arrow">→</b></span></a><a class="public-card" href="/jobs"><span class="public-card-icon">${iconBriefcase({ size: 18 })}</span><h2>Browse jobs</h2><p>Search current roles from the live directory.</p><span class="public-card-meta"><small>Jobs</small><b class="public-card-arrow">→</b></span></a><a class="public-card" href="/companies"><span class="public-card-icon">${iconBuilding({ size: 18 })}</span><h2>Browse companies</h2><p>Explore public company profiles.</p><span class="public-card-meta"><small>Companies</small><b class="public-card-arrow">→</b></span></a></div></div>`;
