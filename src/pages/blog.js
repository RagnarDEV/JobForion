...
// post.body is rich-editor HTML: it is passed through the allow-list
// sanitizer (lib/platform/html-sanitizer.js) at render time, and metadata is escaped.

import { sanitizeRichHtml } from '../lib/platform/html-sanitizer.js';
...
import { getSettings, SETTINGS_DEFAULTS } from '../lib/platform/settings.js';
import { getCategories } from '../lib/content/categories.js';
import { getAdSlotsConfig, DEFAULT_AD_CONFIG } from '../lib/content/ad-slots.js';
...
import { iconFileText, iconInbox } from '../assets/icons.js';
...
export async function renderArticlePage(post, base, env, user = null) {
  ...
  const content = `<div class="page public-page article-page">${PUBLIC_PAGE_CSS}${publicPageHeader({ breadcrumb: bc, eyebrow: post.category || 'CAREER ARTICLE', title: post.title, description: post.excerpt || '' })}<div class="article-meta"><span>${escapeHtml(dateText(post.published_at))}</span>${post.read_time ? `<span>${escapeHtml(post.read_time)}</span>` : ''}<span>By ${escapeHtml(settings.site_name)} Team</span></div>${image ? `<img class="article-cover" src="${image}" alt="${escapeHtml(post.title)}" loading="lazy" decoding="async">` : ''}<article class="article-body">${post.body ? sanitizeRichHtml(post.body) : '<p>This article does not have published body content yet.</p>'}</article>${tags.length ? `<div class="article-tags" aria-label="Article tags">${tags.map(tag => `<span class="article-tag">${escapeHtml(tag)}</span>`).join('')}</div>` : ''}${adSlot('blog-article-footer', 'blog-ad', adConfig, adsEnabled)}${related.length ? `<section class="public-section" aria-labelledby="related-articles"><div class="public-section-heading"><div><h2 id="related-articles">Related articles</h2><p>Other published posts from the Blog CMS.</p></div></div><div class="public-card-grid">${related.map(blogCard).join('')}</div></section>` : ''}${newerPost || olderPost ? `<nav class="article-nav" aria-label="Article navigation">${newerPost ? `<a href="${escapeHtml(postHref(newerPost))}"><small>Newer article</small><strong>${escapeHtml(newerPost.title)}</strong></a>` : '<span></span>'}${olderPost ? `<a href="${escapeHtml(postHref(olderPost))}"><small>Older article</small><strong>${escapeHtml(olderPost.title)}</strong></a>` : '<span></span>'}</nav>` : ''}<div class="public-callout"><div><h2>Keep exploring JobForion</h2><p>Search current roles or read more from the editorial feed.</p></div><div class="public-page-header-actions"><a class="public-primary-link" href="/jobs">Browse jobs</a><a class="public-primary-link" href="/blog">All articles ${iconFileText({ size: 14 })}</a></div></div></div>`;
