...
// NOTE: this function is intentionally NOT async. It only builds and
// returns an HTML string from already-fetched data (job, related) — no
// database or network calls happen here. Callers (pages.router.js) call it
// synchronously and pass the result straight into `new Response(...)`.
// If this function is ever changed to `async function renderJobPage(...)`,
// every call site MUST also add `await`, otherwise `new Response()` ends
// up holding a Promise object instead of the HTML string, and the browser
// renders the literal text "[object Promise]" instead of the page.
