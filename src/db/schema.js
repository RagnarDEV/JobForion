...
// PRAGMA-based column checks (many D1 round-trips) on EVERY single request
// to the entire site — including /sitemap.xml, which made an already-slow
...
export async function ensureTable(env) {
  ...
  // see insertApiSource()/updateApiSource() in db/sync.js.
  await ensureColumn(env, 'api_sources', 'name', 'TEXT');
  // `provider` tells syncJobs() which fetch/mapping logic to use for this
  // row. NOTE: as of the keyless-provider rework, `api_key` no longer
  // holds a real API key for ANY provider — it holds the company's public
  // career-site slug (e.g. a Greenhouse board token, a Lever company
  // slug). The column name is kept as-is to avoid an unnecessary rename
  // migration; only its meaning changed.
  await ensureColumn(env, 'api_sources', 'provider', "TEXT DEFAULT 'greenhouse'");
  // Per-company sync status, surfaced directly in the admin dashboard so
  // a broken or disabled company career page is visible at a glance
  // instead of only showing up buried in the sync log.
  await ensureColumn(env, 'api_sources', 'last_synced_at', 'DATETIME');
  await ensureColumn(env, 'api_sources', 'last_job_count', 'INTEGER DEFAULT 0');
  await ensureColumn(env, 'api_sources', 'last_error', 'TEXT');

  // ── Sync configuration ────────────────────────────────────────
  // Single-row settings table, editable from /admin without a redeploy.
  // sourcesPerRun is the main Cloudflare-free-plan safety knob — see
  // db/sync.js for how it's used.
  await env.DB.prepare(`
    CREATE TABLE IF NOT EXISTS sync_config (
      id INTEGER PRIMARY KEY CHECK (id = 1),
      sources_per_run INTEGER DEFAULT 12,
      jobs_per_company_cap INTEGER DEFAULT 20,
      updated_at DATETIME DEFAULT CURRENT_TIMESTAMP
    )
  `).run();
  await env.DB.prepare(`INSERT OR IGNORE INTO sync_config (id, sources_per_run, jobs_per_company_cap) VALUES (1, 12, 20)`).run();
  ...
  // expires_at: providers don't send a real expiry date, so this is
  // computed by us at insert time (created_at + 45 days) as a best-effort
  // default rather than authoritative source data.
  await ensureColumn(env, 'jobs', 'expires_at', 'DATETIME');
  // source: which provider this job came from (greenhouse, lever, ...)
